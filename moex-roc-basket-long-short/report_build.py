...
def chart_pername(trades):
    ...
    fig.suptitle("Индивидуальные кривые бумаг — OOS, 100 тыс. старт, без плеча, "
                 "лонги и шорты (отсортировано по доходности)", fontsize=12)
...
def chart_signals(ticker, trades):
    ...
    ax.set_title(
        "%s  %s .. %s   (риск 10%%, без плеча, лонги+шорты)\nсделок в окне: %d, "
        "PnL по ним: %s ₽"
        % (ticker, days[0].date(), days[-1].date(), len(sub), f(pnl_win)),
...
def chart_contribution(trades):
    ...
    ax.set_title("Вклад каждой бумаги (OOS, без плеча, лонги и шорты), руб.",
                 fontsize=11)
...
def chart_year_bars(ye):
    ...
    ax.bar(x - w / 2, b, w, label="Стратегия (пул 4,0 млн руб., без плеча, лонги+шорты)",
...
def chart_year_money(ye):
    ...
    ax.set_title("Прибыль по годам (OOS, пул 4,0 млн руб., без плеча, лонги и шорты)",
                 fontsize=11)
    plt.tight_layout()
...
def chart_risk_return(risk_df):
    ...
    ax.plot(risk_df["risk%"], risk_df["per_year_%"], "o-", color="#1257a0",
            lw=2, ms=6, label="Лонги+шорты, без плеча")
    ...
    ax.set_title("Риск → доходность: лонги+шорты (без плеча)", fontsize=11)
...
def chart_leverage(lev_df):
    ...
    ax.set_xlabel("Плечо (кап суммарного номинала к деньгам, риск 10%/сделку)")
    ax.set_ylabel("%/год (пул 4,0 млн руб.)", color="#1257a0")
    ax2.set_ylabel("макс. просадка, %", color="#c0392b")
    ax.set_title("Плечо → доходность (справочно; плечо запрещено): "
                 "лонги и шорты, риск 10%, 40 бумаг (OOS)", fontsize=11)
    ax.legend(loc="upper left", fontsize=9)
    ax2.legend(loc="lower right", fontsize=9)
    plt.tight_layout()
...
MD_RISKS = """1. **Это потолок, а не рекомендация.** Здесь **не моделируются** реальные издержки:
...
def build_reports(res, risk_df, lev_df, ye, bn, st40, lss, lsy, trades40, years):
    ...
    head_rows = [
        ...
        ("Положительных лет",
         "**%s из %s** (единственный минус — 2018)" % (_fnum(years_pos), _fnum(n_years))),
    ]
    ...
    for r in risk_df.to_dict("records"):
        ...
        if r["risk%"] == 10:
            cells = ["**" + c + "**" for c in cells[:3]] + cells[3:]
        risk_rows.append(cells)

    lev_rows = [[_fnum(r["leverage"]) + "×", _pct(r["per_year_%"]),
    ...
    md.append("---")
    md.append("")
    md.append("## 4. Портфель в цифрах (лонги+шорты, без плеча)")
    md.append("")
    ...
    md.append("---")
    md.append("")
    md.append("## 6. Риск → доходность (без плеча)")
    md.append("")
    md.append(_md_table(["Риск на сделку", "% в год", "Просадка пула",
                         "Сделок", "Средняя сделка"], risk_rows))
    md.append("")
    md.append("Доходность **насыщается на ~10% риска** (при больших рисках "
              "номинал = все деньги счёта одной позицией, растёт только "
              "просадка). Рабочая зона — 5–10%.")
    md.append("")
    md.append("![Риск → доходность: лонги+шорты (без плеча)](images/risk_return.png)")
    md.append("")
    md.append("---")
    md.append("")
    md.append("## 7. Плечо (справочно — в этой версии ЗАПРЕЩЕНО)")
    md.append("")
    md.append("Плечо по-прежнему не используется. Таблица ниже приведена только "
              "чтобы показать, сколько «стоит» запрет (и почему решение о запрете "
              "принято): заемный капитал дал бы теоретический +39…+47%/год уже "
              "при плече 2–3×, но реальные издержки маржинального финансирования "
              "лонгов и платы за шорт этот теоретический край съедают (см. §9).")
    md.append("")
    md.append(_md_table(["Плечо", "% в год", "Просадка пула", "Средняя сделка"],
                        lev_rows))
    md.append("")
    md.append("![Плечо → доходность (справочно)](images/leverage_return.png)")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append(("Для иллюстрации работы сигналов на реальных котировках показаны "
               ...
               "OOS-сделки** из walk-forward прогонов (риск 10%%, без плеча, "
               ...
               "прокручивать по горизонтали." % last[:4]))
    md.append("")
    ...
    md.append("---")
    md.append("")
    md.append("## Приложение. Все 40 бумаг (OOS, риск 10%, лонги+шорты, без плеча)")
    md.append("")
    ...
    h.append("<hr>")
    h.append("")
    h.append("<h2>6. Риск → доходность (без плеча)</h2>")
    h.append(_html_table(["Риск на сделку", "% в год", "Просадка пула",
                          "Сделок", "Средняя сделка"], risk_rows))
    h.append("<p>Доходность <strong>насыщается на ~10% риска</strong> (при "
             "больших рисках номинал = все деньги счёта одной позицией, растёт "
             "только просадка). Рабочая зона — 5–10%.</p>")
    h.append("")
    h.append('<img class="normal" src="images/risk_return.png" '
             'alt="Риск → доходность: лонги+шорты (без плеча)">')
    h.append("")
    h.append("<hr>")
    h.append("")
    h.append("<h2>7. Плечо (справочно — в этой версии ЗАПРЕЩЕНО)</h2>")
    h.append("<p>Плечо по-прежнему не используется. Таблица ниже приведена "
             "только чтобы показать, сколько «стоит» запрет (и почему решение о "
             "запрете принято): заемный капитал дал бы теоретический +39…+47%/год "
             "уже при плече 2–3×, но реальные издержки маржинального "
             "финансирования лонгов и платы за шорт этот теоретический край "
             "съедают (см. §9).</p>")
    h.append(_html_table(["Плечо", "% в год", "Просадка пула", "Средняя сделка"],
                         lev_rows))
    h.append("")
    h.append('<img class="normal" src="images/leverage_return.png" '
             'alt="Плечо → доходность (справочно)">')
    h.append("")
    h.append("<hr>")
    h.append("")
    ...
    h.append(("<p>Для иллюстрации работы сигналов на реальных котировках показаны "
              ...
              "(риск 10%%, без плеча, лонги+шорты), попавшие в окно графика, а не "
    ...
    h.append("<hr>")
    h.append("")
    h.append("<h2>Приложение. Все 40 бумаг (OOS, риск 10%, лонги+шорты, без плеча)</h2>")
...
def save_tables(res, trades40, years, tr25):
    ...
    rows_stats = [dict(variant="40_names_L+S_lev1", **st_head)]
    if tr25 is not None and len(tr25):
        rows_stats.append(dict(variant="25_names_L+S_lev1",
                               **trade_stats(tr25)))
...
def main():
    ...
    for risk in RISKS:
        k = ("risk", risk, 1)
        got = get(risk, 1, BASKET40)
        res[k] = (got["folds"], got["trades"], got["nbars"])
    for lev in LEVS:
        k = ("lev", 10, lev)
        got = get(10, lev, BASKET40)
        res[k] = (got["folds"], got["trades"], got["nbars"])
