...
> [!WARNING]
> **Цифры в этом отчёте устарели и не должны цитироваться.**
> Они получены симулятором, который позволял бару, закрывшему позицию по стопу,
> тут же открыть новую **по цене открытия того же бара** — цене, которой уже нет
> в момент срабатывания стопа. На SBER/GAZP/LKOH это 69% всех сделок и 77% прибыли.
> Исправление этого (плюс выходы на границах фолдов, непрогретый ATR и стопы,
> исполнявшиеся по уровню сквозь гэп) снижает результат по 8 бумагам с **+225% до
> +42%** за те же 9,5 лет, и 3 бумаги из 8 уходят в минус.
>
> Симулятор исправлен, но `report/` на нём **не пересобран**. Перед использованием
> цифр: `python report_build.py --no-cache`. Разбор: `CODE_REVIEW.md` §2.1.
...
| Сделок | 7 192 (~66 в месяц по пулу, ~1,6 на бумагу) |
...
| Период OOS | 2017-01 … 2026-08 (9,1 года OOS) |
...
| 2020 | +1 218 694 ₽ | +30,5% | +8,0 | 837 | 53,0 | +1 456 |
| 2021 | +610 943 ₽ | +15,3% | +15,1 | 774 | 50,4 | +789 |
...
|---|---|---|---|---|---|---|
| PIKK | +393 631 ₽ | +393,6 | 187 | +263 066 ₽ | +130 565 ₽ | -88,7% |
...
| SBERP | +185 794 ₽ | +185,8 | 209 | +166 700 ₽ | +19 095 ₽ | -15,7% |
| FEES | +185 363 ₽ | +185,4 | 173 | −14 750 ₽ | +200 113 ₽ | -33,9% |
| SNGS | +182 521 ₽ | +182,5 | 180 | +36 591 ₽ | +145 930 ₽ | -27,7% |
